CREATE TABLE koristuvach (
    id INT PRIMARY KEY,
    imya VARCHAR(100) CHECK (
        REGEXP_LIKE(imya, '^[A-Za-zА-Яа-яЁёІіЇїЄєҐґ\s]{1,100}$')
    ),
    rol VARCHAR(50) CHECK (
        REGEXP_LIKE(rol, '^[A-Za-zА-Яа-яЁёІіЇїЄєҐґ\s]{1,50}$')
    )
);

CREATE TABLE pysmennyk (
    id INT PRIMARY KEY,
    CONSTRAINT pysmennyk_koristuvach_fk FOREIGN KEY (
        id
    ) REFERENCES koristuvach (id)
);
...
CREATE TABLE chernetka (
    ...
    pysmennyk_id INT,
    tvir_id INT,
    CONSTRAINT chernetka_pysmennyk_fk FOREIGN KEY (
        pysmennyk_id
    ) REFERENCES pysmennyk (id),
